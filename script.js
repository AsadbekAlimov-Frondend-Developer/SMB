document.addEventListener('DOMContentLoaded', function () {
    ...
    accordionButtons.forEach(button => {
        button.addEventListener('click', function () {
            const collapseDiv = button.closest('.accordion-item').querySelector('.accordion-collapse');
...
function toggleDropdown(targetId) {
    ...
    const button = document.querySelector(`[onclick="toggleDropdown('${targetId}')"]`);
    ...
    mobileHeaders.forEach(header => {
        ...
        header.addEventListener('click', () => {
            ...
            mobileHeaders.forEach(otherHeader => {
                ...
                if (otherHeader !== header && otherSection && !otherSection.classList.contains('no-dropdown')) {
                    const otherContent = otherSection.querySelector('.smb-mobile-content');
...
function toggleColor(element) {
    ...
    const colorClass = Array.from(element.classList)
        .find(cls => cls !== 'color-option' && cls !== 'active');
    ...
    if (element.classList.contains('active')) {
        ...
        selectedFilters.colors = selectedFilters.colors.filter(color => color !== colorClass);
    }
    ...
    if ($sidebar.length && $container.length) {
        ...
        $(window).scroll(function () {
            ...
            if (window.innerWidth > 768) {
                if (scrollTop > sidebarTop - 20 && scrollTop < containerBottom - sidebarHeight - 40) {
        ...
        $(document).ready(function () {
            ...
            if ($sidebar.length && $container.length) {
                ...
                $(window).scroll(function () {
                    ...
                    let imageHeight = $productImage.length ? $productImage.outerHeight() + 20 : 0;
                    let scrollThreshold = imageHeight * 3;
...
$(document).ready(function () {
    ...
    function switchToStep(step) {
        const actualStep = typeof step === 'string' && ['nominal', 'to', 'from', 'when'].includes(step)
            ? getStepFromTab(step) : step;
    ...
    function validateSenderStep() {
        ...
        const isValid = name.length > 0 && surname.length > 0 &&
            phone.length > 0 && email.length > 0 && terms;
        $('#continue-sender').prop('disabled', !isValid);
    }

    $('#sender-name, #sender-surname, #sender-phone, #sender-email, #terms').on('input change', function () {
    ...
    $(document).on('click', '#close-payment-modal, .close-payment-modal, [data-close="payment-modal"]', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $('#select-payment').click(function () {
        const selectedPayment = $('.payment-options .option.selected').data('payment');
...
$(document).ready(function () {
    ...
    $('.map-pin').on('click', function () {
        $('#selectedPickup').show();
        $('#selectedPickup .pickup-address').text('MSK654, МОСКВА, УЛ. ФЕДОСЬИНО ПУСТА');
        updateCheckoutButton();
    });
...
function validateForm() {
    ...
    if (!firstName || !lastName || !phone || !email || !city || !marketing || !privacy || !paymentSelected) {
        return false;
    }
    ...
    if ($('.city-fields').is(':visible')) {
        ...
        const serviceSelected = $('input[name="deliveryService"]:checked').length > 0;
        ...
        if (deliveryType === 'courier') {
            ...
            const pickup = $('#pickupLocation').val() || $('#selectedPickup').is(':visible');
            if (!pickup) {
                return false;
            }
        }
...
$(document).ready(function () {
    ...
    $(document).ready(function () {
        ...
        function validatePhoneForm() {
            ...
            let checkboxChecked = $(".profile-modal .custom_checkbox_input").is(":checked");
